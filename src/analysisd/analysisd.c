...
#define PII_JSON_PATH "etc/pii.json"
char *hook_url = NULL;    
char *hook_token = NULL;  

int hook();

int hook(const char *input) {
    if (hook_url == NULL || hook_token == NULL) {  
        merror("Hook parameters are null.");       
        return 1;                                  
    }                                              

    if (hook_init() != 0) {
        merror("Failed to initialize hook.\n");
        return 2;
    }
     
    mwarn("Hook Input: %s", input);  // Debug input content

    char *response = NULL;
    
    if (hook_request(hook_url, "POST", hook_token, input, &response) == 0) {
        mwarn("Response:\n%s\n", response);
        if (response)
	    free(response);  // Free the response after using it
    } else {
        merror("Failed to perform request.\n");
    }
...
#ifdef PII_ENABLED
    int res = hook_read_config(PII_JSON_PATH, &hook_url, &hook_token);
    if (res > 0) {
        merror("Failed to load PII configuration: %d", res);
	return 1;
    } 
    mwarn("Pii url: %s\n", hook_url);
#endif
